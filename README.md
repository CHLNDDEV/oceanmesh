...
If you have a different Python distribution, or if you do not want to use packages from conda forge, then the following process may be required:
    ...
    3. Exectute: cmd.exe /C "for %f in (GDAL*.whl Fiona*.whl rasterio*.whl) do pip install %f"
    4. Exectute: pip install geopandas rasterio scikit-fmm pybind11
    5. Exectute: python setup.py install
